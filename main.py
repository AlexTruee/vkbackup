...
class YandexDisk:
    ...
    def create_folder(self, session):
        ...
        try:
            response = session.get(self.HEAD_URL_YD, headers=self.headers_yd, params=params)
        
            if response.ok:
                params.update({'permanently': 'true'})
                session.delete(self.HEAD_URL_YD, headers=self.headers_yd, params=params)
                
            response = session.put(self.HEAD_URL_YD, headers=self.headers_yd, params=params)
    ...
    def loader(self, file_name, file_url, session):
        ...
        try:
            time.sleep(2)
            response = session.post(f'{self.HEAD_URL_YD}/upload', headers=self.headers_yd, params=params)
